...
impl Parser {
    ...
    pub fn parse(&mut self, input: &str) -> Result<&ParsedData, &'static str> {
        let mut start = 0;
            ...
            match self.state {
                ...
                State::Timestamp => {
                    if let Some(end) = input[start..].find(' ') {
                        let date = &input[start..start + end];
                        start += end + 1;
                        if let Some(end) = input[start..].find(' ') {
                            let time = &input[start..start + end];
                            start += end + 1;
                            ...
                            self.data.timestamp =
                                Utc.from_utc_datetime(&datetime).timestamp_millis() as u64;
                            self.state = State::IpPort;
                        } else {
                ...
                State::IpPort => {
                    if let Some(start_idx) = input[start..].find('[') {
                        start += start_idx + 1;
                        if let Some(end_idx) = input[start..].find(']') {
                            let ip_info = &input[start..start + end_idx];
                            start += end_idx + 1;
                            let mut ips = ip_info.split('#');

                            let client_info = ips.next().ok_or("Missing client info")?;
                            let mut client_parts = client_info.split(':');
                            self.data.client_ip =
                                client_parts.next().ok_or("Missing client IP")?.to_string();
                            self.data.client_port = client_parts
                                .next()
                                .ok_or("Missing client port")?
                                .to_string();

                            let server_info = ips.next().ok_or("Missing server info")?;
                            let mut server_parts = server_info.split(':');
                            self.data.server_ip =
                                server_parts.next().ok_or("Missing server IP")?.to_string();
                            self.data.server_port = server_parts
                                .next()
                                .ok_or("Missing server port")?
                                .to_string();

                            self.state = State::DataIdentifier;
                        } else {
                            return Err("Missing IP information closing bracket");
                        }
                    } else {
                        return Err("Missing IP information opening bracket");
                    }
                }
                State::DataIdentifier => {
                    if let Some(end) = input[start..].find(':') {
                        let identifier = &input[start..start + end + 1];
                        self.data.data_identifier = identifier.to_string();
                        start += end + 1;
                        self.state = State::Payload;
                    } else {
                        return Err("Missing data identifier colon");
                    }
                }
                State::Payload => {
                    self.data.payload = input[start..].trim().to_string();
...
fn main() {
    ...
    match parser.parse(log) {
        Ok(parsed_data) => {
            ...
            println!("Client IP: {}", parsed_data.client_ip);
            println!("Client Port: {}", parsed_data.client_port);
            println!("Server IP: {}", parsed_data.server_ip);
            println!("Server Port: {}", parsed_data.server_port);
            println!("Data Identifier: {}", parsed_data.data_identifier);
            println!("Payload: {}", parsed_data.payload);
        }
...
mod tests {
    ...
    fn test_parser() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(1714867221525, parsed_data.timestamp);
        assert_eq!("zjkg".to_string(), parsed_data.client_ip);
        assert_eq!("0".to_string(), parsed_data.client_port);
        assert_eq!("10.0.1.88".to_string(), parsed_data.server_ip);
        assert_eq!("1883".to_string(), parsed_data.server_port);
        Ok(())
    }
}